...
[AI Systems]({{site.glossaryurl}}/#ai-system){:target="_glossary"} must support all the standard practices and techniques for conventional [Cybersecurity]({{site.glossaryurl}}/#cybersecurity){:target="_glossary"}, plus handle new threats.
...
[Agents]({{site.glossaryurl}}/#agent){:target="_glossary"} are potentially-complex [AI Systems]({{site.glossaryurl}}/#ai-system){:target="_glossary"}. Evaluating an individual [LLM]({{site.glossaryurl}}/#large-language-model){:target="_glossary"} for trustworthiness is hard enough; an agent introduces potentially more than one LLM, plus other services, that work together to both return results and sometimes invoke actions on behalf of a user. This increases the challenges of satisfying security concerns.
